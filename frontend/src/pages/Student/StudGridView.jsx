...
function StudGridView() {
  ...
  const handleRegulationClick = (regulationId) => {
    navigate(`/StudGridSyllabusView?regulationId=${regulationId}`);
    };
      ...
      <div className="w-full h-full pt-5 grid grid-cols-2 gap-4">
        {isLoading ? (
          ...
          regulations.map((regulationId) => (
            <button
              key={regulationId}
              className="p-2 rounded-lg text-xl bg-white/80 font-semibold border-none hover:bg-[#65dfc9] transition ease-in-out duration-500"
