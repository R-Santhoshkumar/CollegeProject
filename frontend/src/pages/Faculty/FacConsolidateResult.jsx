...
function FacConsolidateResult() {
  ...
  const fetchCourseCodes = async (event) => {
    ...
    try {
      const response = await axios.get("http://localhost:5000/api/task/fetch_course", {
  ...
  const handleResultClick = (event) => {
    ...
    if (regulation && semester && batch) {
      navigate(`/FacViewConsolidate?regulation=${regulation}&semester=${semester}&batch=${batch}`);
    } else {
...
export default FacConsolidateResult;
