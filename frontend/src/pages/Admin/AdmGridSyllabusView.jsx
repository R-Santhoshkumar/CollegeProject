...
function GridSyllabusView() {
    ...
    const fetchGridSyllabus = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/task/fetch_gridsyllabus?regulation=${regulationId}`
        );
  ...
  return (
    <div className="w-full h-full flex flex-col flex-1 p-5 bg-gradient-to-br from-white/50  to-white/30 backdrop-blur-sm  rounded-r-3xl">
      ...
      <h3 className="mb-2 ">
        Grid Syllabus of Regulation {regulationId}:
      </h3>
      <div
      ...
      >
        {gridSyllabus.length > 0 ? (
          <table className="w-full border-collapse border bg-white/80 border-black">
            <thead>
              <tr>
                {gridSyllabus.length > 0 &&
                  Object.keys(gridSyllabus[0]).map((key) => (
                    key !== "createdAt" && key !== "updatedAt" && (
                      <th
                        key={key}
                        className=" text-center bg-[#65dfc9] font-bold border border-black p-2"
                      >
                        {key}
                      </th>
                    )
...
export default GridSyllabusView;
